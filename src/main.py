        ...
        for subcat in subcategories:
            ...
            for sub_subcat in sub_subcategories:
                ...
                while True:
                    ...
                    for prod in prods:
                        title_prod = prod.find(
                            "h2", class_="c-product-list-item__intitule-slice"
                        ).get_text()
                        # clean title
                        title_prod = title_prod.replace("\n", "").strip()

                        price = (
                            prod.find("p", class_="c-price__amount")
                            .find("span")
                            .get_text()
                        )
                        ...
                        try:
                            price_kg_u = (
                                prod.find(
                                    "p",
                                    class_="c-product-list-item__unit-price u-mt-xs",
                                )
                                .find("span")
                                .get_text()
                            )
                        ...
                        try:
                            nutri = (
                                prod.find(
                                    "div",
                                    class_="c-product-list-item--grid__score-promo-slice",
                                )
                                .find("picture")
                                .find("img")["alt"][-1]
                            )
